...
// Define the policy for whether or not to retry, how long to wait, and how many times
 const getDefaultRetryPolicy = function () {
  return {
    times: 4,
...
// 50ms, 100ms, 200ms, 400ms, etc
 const exponentialBackoff = function (retryCount) {
...
// into one response.
 const combineResultSets = function (setOne, setTwo) {
 ...
 const batchGet = async function (query) {
  return await retry(
 ...
 const batchPut = async function (query) {
  return await retry(
    getDefaultRetryPolicy(),
    async function attemptBatchGet () {
 ...
 const put = async function (query) {
  return await retry(
    getDefaultRetryPolicy(),
...
const transact = async function (query) {
  return await retry(
    getDefaultRetryPolicy(),
    async function attemptQuery () {
...
// Thêm hàm scan mới
const scan = async function (query) {
  return await retry(
    getDefaultRetryPolicy(),
    async function attemptScan () {
      return await DynamoDB.send(new ScanCommand(query))
    }
  )
}
